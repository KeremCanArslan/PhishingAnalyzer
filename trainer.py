...
try:
    # 1. VERİYİ OKU
    # Bazen CSV'ler farklı formatta olabilir, hata almamak için önlem:
    try:
    ...
    # 2. SÜTUNLARI BELİRLE
    # Senin attığın örnekte sütunlar: "Email Text" ve "Email Type"
    # Ama biz yine de kodun bozulmaması için kontrol ekleyelim.
    
    msg_col = 'Email Text'
    label_col = 'Email Type'

    # Eğer sütun isimleri farklıysa (küçük harf vs) otomatik bul
    ...
    # Senin datasetindeki etiketler: "Safe Email", "Phishing Email"
    ...
    # max_features=30000: En sık geçen 30.000 kelimeyi öğrenir.
    ...
    print("\n✅ YENİ MODEL KAYDEDİLDİ!")
    print("Artık main.py dosyasını çalıştırabilirsin.")
    
    # 8. KÜÇÜK BİR TEST (Sağlama)
    print("\n--- ÖRNEK TEST ---")
    ornekler = [
        "Urgent: Your account has been suspended. Click here to verify.", # Phishing
        "Hey, are we still meeting for lunch tomorrow?", # Safe
        "Hesabınız askıya alındı, giriş yapın." # Türkçe Phishing (Bakalım yakalayacak mı?)
    ]
